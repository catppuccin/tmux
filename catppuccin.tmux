...
main() {
  ...
  # NOTE: Check for the value of @catppuccin_right_status_separator
  right_status_separator="$(get-tmux-option "@catppuccin_right_status_separator" "rounded")"
  readonly right_status_separator
  ...
  readonly show_window_rounded="#[fg=$thm_pink,bg=$thm_bg,nobold,nounderscore,noitalics]#[fg=$thm_bg,bg=$thm_pink,nobold,nounderscore,noitalics] #[fg=$thm_fg,bg=$thm_gray] #W #{?client_prefix,#[fg=$thm_red]"
  readonly show_session_rounded="#[fg=$thm_green]}#[bg=$thm_gray]#{?client_prefix,#[bg=$thm_red],#[bg=$thm_green]}#[fg=$thm_bg] #[fg=$thm_fg,bg=$thm_gray] #S "
  readonly show_window_angled="#[fg=$thm_pink,bg=$thm_bg,nobold,nounderscore,noitalics]#[fg=$thm_bg,bg=$thm_pink,nobold,nounderscore,noitalics]  #[fg=$thm_fg,bg=$thm_gray] #W #{?client_prefix,#[fg=$thm_red]"
  readonly show_session_angled="#[fg=$thm_green]}#[bg=$thm_gray]#{?client_prefix,#[bg=$thm_red],#[bg=$thm_green]}#[fg=$thm_bg]  #[fg=$thm_fg,bg=$thm_gray] #S "
  ...
  if [[ "${right_status_separator}" == "rounded" ]] ; then
    local right_column1=$show_window_rounded
  elif [[ "${right_status_separator}" == "angled" ]] ; then
    local right_column1=$show_window_angled
  fi

  # Right column 2 by default shows the current Session name.
  if [[ "${right_status_separator}" == "rounded" ]] ; then
    local right_column2=$show_session_rounded
  elif [[ "${right_status_separator}" == "angled" ]] ; then
    local right_column2=$show_session_angled
  fi
